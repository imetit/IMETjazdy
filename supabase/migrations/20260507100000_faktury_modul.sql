...
-- Plan: docs/superpowers/plans/2026-05-07-faktury-modul.md
-- =====================================================================
...
BEGIN
  ...
  IF OLD.stav = 'uhradena' AND NEW.stav = 'stornovana' THEN
    SELECT EXISTS (
      ...
      WHERE id = COALESCE(NULLIF(current_setting('app.current_user_id', true),'')::uuid, NEW.stornoval_id)
      AND role = 'it_admin'
    ) INTO is_it_admin;
    IF is_it_admin THEN RETURN NEW; END IF;
    RAISE EXCEPTION 'Storno uhradenej faktury vyzaduje it_admin rolu';
  END IF;

  RAISE EXCEPTION 'Neplatny prechod stavov faktury: % -> %', OLD.stav, NEW.stav;
END;
$$ LANGUAGE plpgsql;
